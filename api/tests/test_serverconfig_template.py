...
def template_path() -> Path:
    ...
    return Path(__file__).parent.parent / "src" / "vintagestory_api" / "templates" / "serverconfig-template.json"


@pytest.fixture
def template_data(template_path: Path) -> dict:
...
class TestRequiredKeys:
    ...
    def test_has_required_key(self, template_data: dict, key: str) -> None:
...
class TestDefaultValues:
    ...
    def test_server_name_is_generic(self, template_data: dict) -> None:
        """Server name is a generic default, not a specific server."""
        assert template_data["ServerName"] == "Vintage Story Server"

    def test_port_is_default(self, template_data: dict) -> None:
        """Port is the VintageStory default 42420."""
        assert template_data["Port"] == 42420

    def test_max_clients_is_reasonable(self, template_data: dict) -> None:
...
class TestWorldConfig:
    ...
    def test_seed_is_null_for_random(self, template_data: dict) -> None:
...
class TestModPaths:
    ...
    def test_mod_paths_is_list(self, template_data: dict) -> None:
        """ModPaths is a list."""
        assert isinstance(template_data["ModPaths"], list)

    def test_mod_paths_not_empty(self, template_data: dict) -> None:
        """ModPaths has at least one entry."""
        assert len(template_data["ModPaths"]) > 0

    def test_mod_paths_includes_default_mods(self, template_data: dict) -> None:
